(function() {
  d3.csv('data.cvs', function(error, data) {
    ...
    svg = d3.select('.content').append('svg').attr('class', 'data-container').attr('width', width + 100).attr('height', height + paddingBottom);
    container = svg.append("g").classed("container", true);
    t = new TimelineChart(container);
    return t.xScale(xScale).yScale(yScale).height(height).width(width).data(graphData).draw();
  });
